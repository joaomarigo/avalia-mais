...
$stmtForms = $pdo->query("SELECT id, titulo, criado_em FROM formularios ORDER BY criado_em DESC");
$formularios = $stmtForms->fetchAll(PDO::FETCH_ASSOC);
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* Cards (estética semelhante aos blocos do início) */
    .grid {
...
<body>
  ...
  <div class="content" id="mainContent">
    <div class="actions">
      <h1>Provas</h1>
      <button class="btn-primary" onclick="window.location.href='criar_formulario.php'">
        <i class="fa-solid fa-plus" style="margin-right:6px;"></i> Nova Prova
      </button>
    </div>
    ...
    <div class="grid">
      <?php if (!empty($formularios)): ?>
        <?php foreach ($formularios as $form): ?>
          <div class="card">
            <a class="card-title" href="visualizar_formulario.php?id=<?= (int)$form['id'] ?>">
              <?= htmlspecialchars($form['titulo'] ?: 'Formulário sem título') ?>
            </a>
            <small>Criado em: <?= htmlspecialchars(date('d/m/Y H:i', strtotime($form['criado_em']))) ?></small>
            <a class="more" href="visualizar_formulario.php?id=<?= (int)$form['id'] ?>">Abrir</a>
          </div>
        <?php endforeach; ?>
      <?php else: ?>
        <div class="card">
          <div class="card-title">Nenhuma prova encontrada</div>
          <small>Clique em “Nova Prova” para criar o primeiro.</small>
        </div>
      <?php endif; ?>
    </div>
  </div>
  ...
  <?php if ($sucesso): ?>
    <div class="notify" id="notify">Prova salva com sucesso!</div>
    <script>
      setTimeout(() => {
