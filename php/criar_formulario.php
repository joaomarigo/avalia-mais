...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($titulo === '') {
    ...
    try {
      $stmtIns = $pdo->prepare("INSERT INTO formularios (titulo) VALUES (:titulo)");
      $stmtIns->execute([':titulo' => $titulo]);
      $novoId = $pdo->lastInsertId();

      header("Location: editar_formulario.php?id=" . (int)$novoId);
      exit;
    } catch (PDOException $e) {
      $erro = 'Erro ao salvar prova.';
    }
  }
}
?>
...
<head>
  ...
  <title>Criar Prova - Avalia+</title>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* Card (mesma estética dos blocos brancos) */
    .card {
    ...
    .input_text {
      width: 100%; padding: 10px 5px; border: none; border-bottom: 2px solid #12337b;
      background: transparent; outline: none; font-size: 20px; color: #12337b;
    }
    ...
    .btn-primary {
      height: 44px; padding: 0 18px; border: 2px solid #1a3a7c; border-radius: 25px;
      background: transparent; color: #1a3a7c; font-size: 16px; cursor: pointer; transition: .2s;
    }
...
<body>
  ...
  <div class="content">
    <div class="card">
      ...
      <?php if ($erro): ?>
        <div class="flash err" style="margin-bottom:14px;"><?= htmlspecialchars($erro) ?></div>
      <?php endif; ?>

      <form method="POST" action="">
        <label class="label" for="titulo">Título</label>
        <input class="input_text" id="titulo" type="text" name="titulo" placeholder="Ex.: Avaliação de Matemática - 2º B" required>

        <div class="buttons">
          <button type="button" class="btn-primary" onclick="window.location.href='formularios.php'">Voltar</button>
